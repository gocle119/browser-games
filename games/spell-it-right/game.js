(() => {
  const GAME_ID   = 'spell-it-right';
  const LIVES_MAX = 3;
  const TIME_BY_DIFF = { 1: 10000, 2: 8000, 3: 6000 };
  ...
  function nextWord() {
    ...
    // Shuffle correct word among misspellings, track which slot it lands in
    displayOpts = shuffle([p.word, ...p.misspellings]);
    correctSlot = displayOpts.indexOf(p.word);

    tiles.forEach((tile, slot) => {
      tileWords[slot].textContent = displayOpts[slot];
      tile.className = 'tile';
      tile.disabled  = false;
    });
  ...
  function showFeedback(correct, puzzle, pts, speedBonus, timedOut = false) {
    ...
    feedbackConn.textContent = 'Correct: ' + puzzle.word;
