(() => {
  ...
  const tileLetters  = [0,1,2,3].map(i => document.getElementById('letter-' + i));
  ...
  function currentEntry() { return state.words[state.index]; }

  function nextWord() {
    if (state.index >= state.words.length) { endGame(); return; }
    answered = false;

    const p = currentEntry();
    const correctLetter = p.word[p.blankIndex];
    const isVowel = VOWELS.includes(correctLetter);
    const pool = (isVowel ? VOWELS : CONSONANTS).filter(l => l !== correctLetter);
    const distractors = shuffle(pool).slice(0, 3);
    choiceLetters = shuffle([correctLetter, ...distractors]);
    correctSlot = choiceLetters.indexOf(correctLetter);

    tiles.forEach((tile, slot) => {
      tileLetters[slot].textContent = choiceLetters[slot];
      tile.className = 'tile';
      tile.disabled  = false;
    });

    wordDisplay.innerHTML = [...p.word].map((ch, i) => {
      if (i === p.blankIndex) return '<span class="letter-box blank">_</span>';
      return '<span class="letter-box">' + ch + '</span>';
    }).join('');

    roundLabel.textContent = state.index + 1;
  ...
  function startTimer() {
    ...
    timerInterval = setInterval(() => {
      if (!answered && Date.now() - timerStart >= TIME_LIMIT) {
        clearInterval(timerInterval);
        handleAnswer(-1);
      }
    }, 200);
  ...
  function handleAnswer(tappedSlot) {
    ...
    const correct = tappedSlot === correctSlot;

    if (correct) {
      const speedBonus = elapsed < TIME_LIMIT * 0.4;
      ...
      tiles[correctSlot].classList.add('correct');
  ...
  function showFeedback(correct, entry, pts, speedBonus, timedOut = false) {
    ...
    feedbackConn.textContent = 'Word: ' + entry.word;
  ...
  // Tap feedback bar to skip delay
